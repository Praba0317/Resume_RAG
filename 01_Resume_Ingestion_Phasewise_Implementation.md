...
- The collection configured by `COLLECTION_NAME` can be accessed.
...
Store structured resume metadata and the generated embedding in the collection configured by `COLLECTION_NAME`.
...
The collection name must be read from `.env` through the shared environment configuration. Do not hardcode `"resumes"` in the repository. Use `resumes` only as the documented default value when `COLLECTION_NAME` is not set.
...
- Document exists in the collection configured by `COLLECTION_NAME`.
...
```json
{
  "ingestionReady": true,
  "server": "same-backend",
  "collection": "value of COLLECTION_NAME",
...
Mistral Resume Embedding
    ↓
MongoDB collection configured by `COLLECTION_NAME`
    ↓
...
That document must reuse this same project, MongoDB connection, collection configured by `COLLECTION_NAME`, stored metadata, and stored resume embeddings. Do not create a second backend server or a second codebase.
